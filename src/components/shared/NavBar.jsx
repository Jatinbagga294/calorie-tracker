...
export default function NavBar() {
  return (
    <nav className="fixed bottom-0 inset-x-0 border-t border-slate-200/80 dark:border-slate-800 bg-white/90 dark:bg-slate-950/90 backdrop-blur-md pb-[env(safe-area-inset-bottom)] z-20">
      <div className="max-w-lg mx-auto grid grid-cols-5 px-2 pt-1.5 pb-1">
        {TABS.map(({ to, label, Icon, end }) => (
          <NavLink key={to} to={to} end={end} className="flex flex-col items-center gap-0.5">
            {({ isActive }) => (
              <>
                <span
